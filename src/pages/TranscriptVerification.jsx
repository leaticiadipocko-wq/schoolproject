...
export default function TranscriptVerification() {
  ...
  const { user } = useAuth()
  const { results: allResults, signatures = {}, users = [] } = useData()
  ...
  useEffect(() => {
    ...
    const foundStudent = users.find(u => u.studentId === studentId)
    ...
    if (studentResults.length === 0) {
      // Use mock results if no specific results found
      const mockResults = allResults.filter(r => !r.studentId)
      setResults(mockResults)
      const cgpaCalc = mockResults.length
        ? (mockResults.reduce((s, r) => s + (gradePoints[r.grade] || 0), 0) / mockResults.length).toFixed(2)
        : '0.00'
      setCgpa(cgpaCalc)
    } else {
      setResults(studentResults)
      const cgpaCalc = studentResults.length
  ...
  const handleDownload = async () => {
    ...
    try {
      ...
      pdf.save(`SIARM_Transcript_Verification_${studentId}.pdf`)
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-ink-50">
        <div className="flex flex-col items-center gap-4">
          ...
          <p className="text-ink-600">Verifying transcript...</p>
        </div>
      </div>
    )
  }
      ...
      <main className="max-w-4xl mx-auto px-4 py-8">
        ...
        <div className="mb-6 p-4 rounded-xl bg-emerald-50 border border-emerald-200 flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-emerald-600">This transcript has been verified against IUGET official records</p>
          </div>
        </div>
        ...
        <div ref={ref} className="bg-white rounded-2xl shadow-soft border border-ink-100 p-10">
          ...
          <div className="mt-12 pt-6 border-t border-ink-200 grid grid-cols-3 gap-8 items-end">
            ...
            <div className="flex flex-col items-center">
              <div className="bg-white p-1 rounded-lg border border-ink-200 shadow-soft">
                <QRCode value={`${window.location.origin}/verify/transcript/${studentId}`} size={100} />
              </div>
          ...
          <div className="mt-8 text-[10px] text-ink-400 text-center flex items-center justify-center gap-1.5">
            <FileText size={10} /> Generated by SIARM · IUGET Bonabéri · Verify authenticity at {window.location.origin}/verify/transcript/{studentId}
          </div>
        </div>
